...
CompressedRelationReader::asyncParallelBlockGenerator(
  ...
  auto generator{asyncParallelBlockGeneratorIR(
      beginBlock, endBlock, scanConfig, cancellationHandle, limitOffset)};

  return fromInputRange(std::move(generator));
}

template <typename T>
CompressedRelationReader::IdTableGeneratorIR
CompressedRelationReader::asyncParallelBlockGeneratorIR(
    T beginBlock, T endBlock, const ScanImplConfig& scanConfig,
    CancellationHandle cancellationHandle,
    LimitOffsetClause& limitOffset) const {
  // Empty range.
  if (beginBlock == endBlock) {
    return IdTableGeneratorIR{};
  }
  ...
  struct Generator
    ...
    auto decompressAndPostprocessBlockF{
        std::bind(&CompressedRelationReader::decompressAndPostprocessBlock,
  ...
  bool CompressedRelationReader::FilterDuplicatesAndGraphs::
    ...
    if (!blockMetadata.containsDuplicatesWithDifferentGraphs_) {
      AD_EXPENSIVE_CHECK(std::unique(block.begin(), block.end()) ==
                         block.end());
      return false;
    }
  ...
  IdTable CompressedRelationReader::scan(
    ...
    for (const auto& block :
         lazyScan(scanSpec,
                  convertBlockMetadataRangesToVector(
  ...
  DecompressedBlock CompressedRelationReader::readPossiblyIncompleteBlock(
    ...
    auto filterColumn = [&block, &beginIdx, &endIdx](
      ...
      auto matchingRange =
          ql::ranges::equal_range(column.begin() + beginIdx,
    ...
    for (const auto& inputColIdx :
         columnIndices | ql::views::filter([&](const auto& idx) {
           return !manuallyDeleteGraphColumn ||
                  idx != ADDITIONAL_COLUMN_GRAPH_ID;
         })) {
  ...
  std::pair<size_t, size_t> CompressedRelationReader::getResultSizeImpl(
    ...
    auto readSizeOfPossiblyIncompleteBlock = [&](const auto& block) {
      if (exactSize) {
        ...
        bool isComplete =
            isTripleInSpecification(scanSpec, block.firstTriple_) &&
  ...
  size_t CompressedRelationReader::getNumberOfBlockMetadataValues(
    ...
    return std::transform_reduce(blockMetadata.begin(), blockMetadata.end(),
                                 0ULL, std::plus{}, ql::ranges::size);
  };
  ...
  BlockMetadataRanges CompressedRelationReader::getRelevantBlocks(
    ...
    ql::ranges::for_each(
        blockMetadata, [&resultBlocks, &key, &comp](
                           const BlockMetadataRange& blockMetadataSubrange) {
          auto result =
              ql::ranges::equal_range(blockMetadataSubrange, key, comp);
        ...
        AD_CONTRACT_CHECK(
            !b1.containsInconsistentTriples(firstFreeColIndex), [&] {
              return createErrorMessage(
                  b1, b2,
                  absl::StrCat(
                      "The following First Block contains non-constant "
